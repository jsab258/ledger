...
# ---------------------------------------------------------------------------
# WHICH PROMPT FILE THIS RUN IS ABOUT. ONE IMPLEMENTATION, and it used to be
# THREE COPIES of the same expression - the selftest, the dry run and the batch
# each named `prompts.json` beside this file (lines 2929, 4482 and 4599 before
# this block existed). That made two things impossible. A one-off spec could
# not be run at all; and anything written into the library to get it generated
# was then generated by EVERY later night, landed in the shipped decals
# directory, and was regenerated by the resume rule whenever its recipe moved.
#
# THE DEFAULT IS THE LIBRARY, so a caller that passes nothing reads exactly the
# file it read before. That is the case the selftest checks FIRST, as a whole
# dry run compared character for character against a run with no flag: a new
# path through an old default is where a default quietly changes.
# ---------------------------------------------------------------------------
DEFAULT_SPEC = pathlib.Path(__file__).parent / "prompts.json"
SPEC_MISSING_EXIT = 7      # distinct from 2 disk, 4 nothing-made, 5 gate, 6 blank


def spec_file(given=None):
    """The spec path this run reads: `--spec` when given, else the library."""
    g = str(given).strip() if given is not None else ""
    return pathlib.Path(g) if g else DEFAULT_SPEC


def load_spec(given=None):
    """(spec|None, path, state). THREE STATES, not one falsy answer.

    `absent` (nobody wrote a file there), `unreadable:<ErrorName>` (a file that
    is not JSON) and `read` are three different faults with three different
    fixes, and a caller that cannot tell them apart prints one wrong sentence
    for two of them. Same shape as `_read_manifest` and `_read_path_list`,
    which are the two places this project already learned it.
    """
    p = spec_file(given)
    if not p.exists():
        return None, p, "absent"
    try:
        return json.loads(p.read_text(encoding="utf-8")), p, "read"
    except Exception as e:                                    # noqa: BLE001
        return None, p, f"unreadable:{type(e).__name__}"


def spec_line(path, given=None, spec=None):
    """ONE key=value line naming the spec that won AND the source that chose
    it, in the shape `batch_settings` already prints: the value and its
    provenance on the same line, because a path whose source is not printed
    cannot be told from a path that came from the wrong place.

    The item count travels here too. It is the denominator every later "0
    problems" in this run is a numerator over.
    """
    n = len((spec or {}).get("items") or []) if spec else 0
    return (f"spec file={_nospace(path)} "
            f"source={'--spec' if (given and str(given).strip()) else 'default-library'} "
            f"schema={(spec or {}).get('schema', 'unknown')} itemsInSpec={n}")


def refuse_missing_spec(path, state, say=print):
    """The words a never-ran case must print, and the exit code nobody else
    uses. A run that read no spec examined NOTHING: it must not be able to read
    as a clean run with no problems, so it says `nothing measured` in those
    words and returns 7 rather than 0.
    """
    say(f"spec file={_nospace(path)} state={state} itemsExamined=0 "
        "problems=nothing-measured")
    say(f"REFUSED: nothing measured. There is no readable spec at {path} "
        f"(state: {state}), so zero items were examined and zero pictures were "
        "generated. Check the --spec path, or pass no --spec at all to read "
        f"the library at {DEFAULT_SPEC}.")
    return SPEC_MISSING_EXIT
...
# `spec` and `out` JOINED THE SENTINEL ON 9 SEP, for the one-off comparison
# board: a spec that is not the shipped library, generated into a directory that
# is not the shipped decals directory, asked for in the same commit that starts
# the run. EMPTY MEANS THE OLD BEHAVIOUR in both cases - the library beside the
# tool and the default output directory - so a sentinel that mentions neither
# starts exactly the run it started yesterday.
#
# AND A BAD `spec` DOES NOT FALL BACK, which is the opposite of the rule above
# it and deliberately so. `limit` fails back to the SMALL batch, which is safe.
# A spec path nobody can read falling back to the library would generate the
# whole 45-item library into whatever `out` says, which is the expensive
# direction. So the value is passed through, and the tool refuses it at the file
# (exit 7, "nothing measured") before the probe and before the 6.7 GB.
...
def batch_settings(sentinel_text="", limit_in="", only_in="", max_minutes_in="",
    ...
    Returns (values, notes): `values` is the five env keys the workflow
    exports (three since 2 Sep, `spec` and `out` added 9 Sep), `notes` is one
    printed line per key naming the SOURCE and, when a value was refused,
    capped or left empty, why. The workflow supplies only live state
    ...
    given = {"limit": (limit_in or "").strip(),
             ...
             "spec": (spec_in or "").strip(),
             "out": (out_in or "").strip()}
    for key in ("limit", "only", "max_minutes", "spec", "out"):
        if given[key]:
        ...
        if key == "limit":
            ...
            # THE VALUE IS A PATH AND IS NOT MANGLED. `_nospace` is for the
            # printed note only: a Windows path can legitimately carry a space,
            # and hyphenating it would hand the tool a file that does not exist
            # while the log showed something that looked fine.
            if val and not val.lower().endswith(".json"):
                why = ("does-not-end-.json-the-run-will-refuse-it-and-exit-"
                       f"{SPEC_MISSING_EXIT}")
            elif not val:
                why = "empty-means-the-library-beside-the-tool"
        elif key == "out":
            if not val:
                why = "empty-means-the-default-decals-directory"
        values[key] = val
...
def selftest(spec_arg=None):
    """Both outcomes of everything in this file, accepting case first.

    `spec_arg` is `--spec`, so `--selftest --spec <file>` checks THAT file's
    items the way the default run checks the library's. A path nobody can read
    refuses here too, with the same words and the same exit code as the batch:
    a validator that quietly tested nothing and printed a summary of zero
    checks is the silent-instrument failure this file exists to avoid.
    """
    spec, spec_p, spec_state = load_spec(spec_arg)
    if spec is None:
        print("imagegen selftest")
        print("-" * 60)
        refuse_missing_spec(spec_p, spec_state)
        print("-" * 60)
        print("  0 passed, 0 failed, 0 checks run - nothing measured, because "
              "the spec every check below reads could not be read")
        return SPEC_MISSING_EXIT
    ok = fail = 0
    ...
    # THE SPEC UNDER TEST was read at the top of this function, from `--spec`
    # or from the library beside the tool. It is named in the checks below, so
    # a green line can never be read as being about a file it did not open.
    check(f"accepting case: the spec under test was read, and the line names "
          f"it and counts what it holds ({spec_line(spec_p, spec_arg, spec)})",
          spec_state == "read" and (spec.get("items") or []),
          f"state={spec_state}")
    rules = spec["content_rules"]["rules_clause"]
...
def main():
    ...
    ap.add_argument("--spec", default="",
                    help="the prompt spec to read. Default: prompts.json "
                         "beside this tool, which is the shipped library. A "
                         "ONE-OFF spec belongs in its own file with its own "
                         "--out: an item added to the library is generated by "
                         "every later night, lands in the shipped decals "
                         "directory, and is regenerated whenever its recipe "
                         "moves. Every guard runs unchanged on whatever is "
                         "named here; a path nobody can read is refused with "
                         "the words 'nothing measured' and exit "
                         f"{SPEC_MISSING_EXIT}, before the probe and before "
                         "the 6.7 GB.")
    ...
    if a.batch_settings:
        ...
        values, notes = batch_settings(
            ...
            os.environ.get("INPUT_SPEC", ""),
            os.environ.get("INPUT_OUT", ""))
        for line in notes:
            print(line)
        ...
        out = [f"BATCH_LIMIT={values['limit']}",
               ...
               f"BATCH_SPEC={values['spec']}",
               f"BATCH_OUT={values['out']}"]
        if env:
    ...
    # THE SPEC, READ ONCE AND AS EARLY AS IT CAN BE READ. Both sites that used
    # to load it (the dry run's first command and the batch) are downstream of
    # here, so there is one read per run and the answer cannot differ between
    # them. EARLY because a path nobody can read must cost a printed line, not
    # a probe, a plan, a report and then a traceback on the way into the batch.
    # The verdict and --staged-files above never read a spec at all: they
    # measure the manifest and the PNGs on disk, which is the whole point of
    # them, so they are answered before this.
    spec, spec_p, spec_state = load_spec(a.spec)
    if spec is None:
        return refuse_missing_spec(spec_p, spec_state)
    ...
    # WHICH SPEC THIS RUN IS ABOUT, IN THE RUN'S OWN LOG AND IN THE REPORT,
    # because the report tail is the only part of a night a person reads and
    # "which prompt file" is now a question with more than one answer.
    log(spec_line(spec_p, a.spec, spec))
    ...
    outdir.mkdir(parents=True, exist_ok=True)
    # `spec` was read once, near the top of main, from `--spec` or the library.
    only = [x.strip() for x in a.only.split(",") if x.strip()] if a.only else None
    log("")
    log(f"Generating into {outdir} from {spec_p}")
