...
REM
REM BACK FROM THE ARCHIVE, 2026-09-22, and the reason is worth the four lines.
REM When the studio was paused, tools/runner/ went to legacy/ with the rest of
REM its machinery - but the Unreal probe workflow still CALLS this file, on its
REM second step, before anything else. So every push since the pause failed
REM here, the build step was skipped, and THE UNREAL BUILD HAS NOT RUN ONCE
REM SINCE. The runs were red, which is the honest colour, and red for a reason
REM that had nothing to do with the build: the safeguard hardened last sitting
REM was standing behind a door that never opened. This file is not studio
REM administration; it is the two lines of PATH the build itself needs.
REM
