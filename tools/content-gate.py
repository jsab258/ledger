...
#
# AND THE DATED RECORDS, 24 September: specs and deliveries of pictures that
# already exist (or of one-off runs never made), kept as written so they stay
# true about what was asked. Rewording them would falsify the record, and a
# re-run is still refused by imagegen's own forbidden_tokens. What the drawn
# pictures show is a person's check, never this gate's: see --enforceable.
EXEMPT = ("canon.md", "ledger-v2/", "legacy/", "tools/content-gate.py",
          "production/specs/judge-",
          # asked for a beer firkin; drew production/art/compare/hook-2026-09-09/hook_compare*.png
          "tools/imagegen/compare-hook-2026-09-09.json",
          # the same, pass 2; drew production/art/compare/hook-2026-09-09-pass2/hook_pass2*.png
          "tools/imagegen/compare-hook-2026-09-09-pass2.json",
          # asked for a school yard and a satchel; drew concept-fairview-2026-09-10/sheets/fairview_sheet*.png
          "production/art/concept-fairview-2026-09-10/fairview-sheet-2026-09-10.json",
          # lettering composited onto sheets/fairview_sheet_finished.png, satchel tile included
          "production/art/concept-fairview-2026-09-10/fairview-furniture-2026-09-10.json",
          # the provenance table of that sheet's prompt (school yard, satchel), as delivered
          "production/art/concept-fairview-2026-09-10/DELIVERY.md",
          # never run (a pram in the prompt, which its own forbidden_tokens refuses)
          "production/art/concept-copper-row-2026-09-10/copper-row-sheet-2026-09-10.json",
          # never run (a satchel in the prompt, which its own forbidden_tokens refuses)
          "production/art/concept-fairview-2026-09-10/fairview-pair-2026-09-11.json",
          # pub research delivered 2026-09-08 (casks, beer drop, free house), voided by D17
          "production/art/atlas-02/DELIVERY.md")
...
# EMPTIED 2026-09-24 by the content pass: every line rewritten, and the
# dated art records moved to EXEMPT above, each with its reason.
...
#: THE SPEECH RULES AS C#, 23 September, for the one text nobody writes: a
#: model's reply in conversation. Offered a drink, the paid model's Sam went
#: for one; the gate reads the banks, the barks and the cards, and a reply is
#: none of those. So the game checks every reply against THESE rules at run
#: time (Core/ContentWords.cs via ContentRule.SpeechBreaks), and there is one
#: list, here: the C# file is generated from it and --emit-core --check fails
#: when the two differ.
CORE_WORDS_REL = os.path.join("ledger", "Assets", "Scripts", "Core", "ContentWords.cs")


def core_words_source():
    """The generated C# for every speech rule, in RULES order."""
    rows = []
    for d, _rx in RULES:
        if not d.get("speech", True):
            continue
        # A C# VERBATIM STRING, so the pattern's backslashes are left as the
        # gate wrote them and only a double quote is doubled.
        rows.append('            new Rule("%s", "%s", %s, @"%s"),' % (
            d["kind"], d["id"], "true" if d.get("cased") else "false", d["pattern"].replace('"', '""')))
    return "\n".join([
...
def main():
    ...
    ap.add_argument("--emit-core", action="store_true",
                    help="write the speech rules as Core/ContentWords.cs (with --check: compare only)")
    ap.add_argument("--check", action="store_true",
                    help="with --emit-core: fail if the committed C# differs from the rules")
    a = ap.parse_args()

    if a.emit_core:
        path = os.path.join(REPO if "REPO" in globals() else os.path.dirname(os.path.dirname(os.path.abspath(__file__))), CORE_WORDS_REL)
        want = core_words_source()
        have = ""
        try:
            ...
            pass
        n = sum(1 for d, _ in RULES if d.get("speech", True))
        if a.check:
            same = have == want
            print("contentWords file=%s rules=%d current=%s" % (CORE_WORDS_REL.replace(os.sep, "/"), n, "yes" if same else "NO"))
            return 0 if same else 6
        with open(path, "w", encoding="utf-8", newline="\n") as fh:
            fh.write(want)
        print("contentWords wrote=%s rules=%d" % (CORE_WORDS_REL.replace(os.sep, "/"), n))
        return 0
