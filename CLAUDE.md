...
- EVERY MESSAGE BEGINS WITH A LINE THAT READS `For you:` — followed either by the word `nothing`, or by the items, one to a line. It goes FIRST, before anything else, EVERY TIME, including when it is empty. Ruled 2026-09-22 after the rule it replaces failed twice in one sitting.
...
- Do not build anything whose purpose is to measure, report on, or enforce this session's own behaviour. ONE EXCEPTION, ruled by Jafar on 22 September and the only one: a stop hook, with TWO jobs and no more. (1) It will not let a turn end while the standing list has unfinished items and the sitting's time limit has not passed, because a rule in a document cannot stop a turn from ending. (2) It will not let a turn end whose last message does not begin `For you:`, for the same reason and after the same failure. It reads NOW.md and the message, keeps no records and no counters, and fails open. WRITE THE SITTING'S START TIME AND LIMIT INTO NOW.md AT THE START OF EVERY SITTING, on its own line as `SITTING: started <time with offset>, limit <N>h`; without it the hook permits every stop and says it measured nothing. Do not give it a third job, and do not add a second hook.
