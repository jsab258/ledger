...
- The steady packaged copy lacks the files the build machine lays beside the game (the piece list, the witness lines, the street's look): played from there the street is grey, with placeholder surfaces and torn rooftops (the AI tester, 19:07). The game can now be told where the repository is (-LedgerRepo), which the tester passes; not yet seen working on the packaged copy. The play file uses the editor's build meanwhile.
