...
| Crime (the deed) | A hidden glass piece with collision off, read back after the call; eight shard boxes and a brick box on traced ground; a forced sequence frame either side so the break reads as a cut. | A projectile that crosses the frames and a break sound in a clip format that carries sound; `Observe.DeedFor` over an `Arsenal` row for a half-brick so the `Deed` is the game's and not hand-built (queues 150, 151). |
