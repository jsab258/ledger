...
The text that follows is therefore an EDITED copy of the 04:09 prompt:
    ...
    THEN: BUDGET FIRST, from `production/budget.md`, which is the authority and this prompt is not. THE STANDING CEILING IS 85 ON THE HIGHER METER, ruled by Jafar 2026-09-10, and it is 85 here only so a session that never opens the file is not told a repealed number; the file still wins if the two ever disagree. It stood at 80 until that ruling and this prompt said 80 until 2026-09-13, which is the same stale-copy fault the file's own header paragraph was written about. Read BOTH meters, the higher one governs, unless Jafar has set a lower ceiling for the session in his own words, in which case his number wins and the budget file records it. DO NOT CLAIM TO MONITOR OR ENFORCE A LIVE PERCENTAGE: nothing in the container can read his usage page. Ask for a fresh reading at the existing checkpoints instead. If a stop condition holds, say so in the brief and do not start work; an unknown budget is not permission.
...
THE STALE BLOCK IT REPLACED, kept so the drift is readable:
    ...
    1. BUDGET FIRST. Read `production/budget.md`. It is the authority and this prompt is not. If a stop condition holds, say so in the brief and do not start work. An unknown budget is not permission. THE STANDING CEILING IS 85 ON THE HIGHER METER, ruled 2026-09-10; read BOTH meters, the higher one governs. Do not take the 85 from here if the file says otherwise: the file wins, and this line said 80 for three days after the ruling.
