# For Jafar

One dated summary per sitting, written at its end, under 200 words: what
changed, the evidence, what failed or is unproven, what is next, and any
decision you need to make. Unresolved decisions carry forward. Earlier
summaries are in git; everything before 24 September afternoon is in
production/archive/FOR-JAFAR-to-2026-09-24.md.

## 24 September, night: the cast's faces, pose and portraits

**Portraits beside the KCD2 people frame.** Recast: [Lena](production/d1-probe/portraits/lena-cast-t2-2026-09-24.jpg), [Sam](production/d1-probe/portraits/sam-cast-t2-2026-09-24.jpg), [Rocco](production/d1-probe/portraits/rocco-cast-t2-2026-09-24.jpg). Before: [Lena](production/d1-probe/portraits/lena-stand-ins-2026-09-24.jpg), [Sam](production/d1-probe/portraits/sam-stand-ins-2026-09-24.jpg), [Rocco](production/d1-probe/portraits/rocco-stand-ins-2026-09-24.jpg).
Fixed: hands through bodies, frozen faces, wrong casting. Still wrong: barefoot in T-shirts; Lena's hair dark, not grey. Why: the clothes need your Fab sign-in; my grey didn't take. A third take ([Lena](production/d1-probe/portraits/lena-cast-t3-2026-09-24.jpg)) swapped the haircut cleanly but looked worse, so the game keeps the second.

**Changed.**
- Lena looked Chinese because I'd started everyone from Epic's 29 ready-made faces, and none is an older British woman. Each is now blended to a brief: face, skin, aged skin, build, hair.
- They idle in Epic's own body and face loop.
- Face textures were 2K; now 4K.

**Unproven.** No comparison with Epic's MetaHuman sample yet. Canon's overcast street lights faces softer than KCD2's sun.

**Next, overnight.** Grey hair, no eye make-up on Lena, clothes and shoes, the sample.

**You:** sign in to fab.com in my browser pane; I can't.

**Decide** the brief:
- (A) Lena British, about 60, small; Rocco British-Italian, late 50s, heavy; Sam Scottish, late 20s, wiry (recommended).
- (B) You set each one's background and age.
- (C) You write it.

## 24 September, evening: suspicion, the cast as MetaHumans, the encounter to play

**Changed.**
- Lena saw Tom's face and Sam saw him run, so they question him live. Rocco only heard, so he doesn't.
- Rocco, Lena and Sam are MetaHumans, about 0.3 ms each; the slice holds 73 fps at your screen size.
- The encounter is playable with them, and the build plays it too. You type what you say, they answer aloud in their cast voices, and the town remembers after you quit.
- The router got worked examples and blocks typed orders.
- An AI tester plays and reports: six runs, $2.19.

**Failed.** No route put a donkey jacket on a MetaHuman (written up in the clothing research). The cast are barefoot in T-shirts.

**Play it.** Double-click tools\play\PLAY THE ENCOUNTER FROM THE START.bat.
- WASD to walk.
- E at Mickey's window.
- T to talk.
- Esc to leave.

**Decide.** May I download Epic's four free MetaHuman clothing items from Fab (T-shirt, slim jeans, hoodie, crowd sample) to dress the cast plainly?
- (a) Yes (recommended).
- (b) No, wait.

### What the AI tester found (six runs, $2.19)

Worst first; all fixed.
- It could not talk to "Lena": it was a stand-in woman at Mickey's door. Fixed: the stand-ins the cast replaces are gone from the encounter, and "too far" now says who is nearest.
- It never found the yard. Fixed: the game now says where the yard is.
- It broke the window and saw nothing happen. Fixed: a line says so, and glass lies on the pavement.
- Its camera went inside a MetaHuman. Fixed: they now block the player.
- Its first picture caught another app on your desktop. Fixed: it now copies only the game window. That picture stays on this PC.
- On the steady packaged copy it found the street grey: that copy could not find the street's files. Fixed: the game is told where they are, and a last run shows the real street.
- A false alarm: it took Lena for a child. She is an adult, only shorter than Tom.

### AI tester, 2026-09-24 19:31

12 steps, 1 minutes, $0.11. Not a gate. Worst first:
- (2) Scene is very dark, hard to see building details like Mickey's minicab sign; lighting may be too low.
- (2) Overall scene lighting remains extremely dark throughout play session, making it hard to identify shop signs (e.g., Mickey's minicab office) and NPCs; likely a lighting/exposure bug.
Full report: production/playtest/ai-tester/2026-09-24-1931/report.md
